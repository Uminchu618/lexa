...
class Dreamer(tools.Module):
    ...
    def _policy(self, obs, state, training, reset, should_expl=False):
        ...
        feat, latent = self._wm.get_init_feat(
            obs,
            state,
            sample=self._config.collect_dyn_sample and not self._config.eval_state_mean,
        )
        ...
        if not training:
            ...
            if self._config.offpolicy_use_embed:
                action = self._off_policy_handler.actor(
                    tf.concat(
                        [
                            self._wm.encoder(obs),
                            self._wm.encoder({"image": obs["image_goal"]}),
                        ],
                        axis=-1,
                    )
                )
    ...
    def _train(self, data):
        ...
        if (
            ...
            and self._config.dd_train_off_policy
        ):
            metrics.update(
                self._task_behavior.train_dd_off_policy(
...
def make_env(
    config, logger, mode, train_eps, eval_eps, use_goal_idx=False, log_per_goal=False
):

    if "dmc" in config.task:
        suite, task = config.task.split("_", 1)
        env = envs.DmcEnv(
            task, config.size, config.action_repeat, use_goal_idx, log_per_goal
        )
        ...
        dmc_envs = list(
            ...
            for task in ["walker_walk", "quadruped_run"]
        )

        env = envs.MultiplexedEnv(
            [kitchen_env, robobin_env] + dmc_envs,
...
def create_envs(config, logger):
    ...
    make = functools.partial(
        make_env, config, logger, train_eps=train_eps, eval_eps=eval_eps
    )
    train_envs = [make("train", log_per_goal=True) for _ in range(config.envs)]
    eval_envs = [
        make("eval", use_goal_idx=True, log_per_goal=config.test_log_per_goal)
