        ...
        with tf.GradientTape() as model_tape:
            ...
            if self._config.latent_constraint == "consecutive_state_l2":
                ...
                for i in range(seq_len - 1):
                    model_loss += 0.1 * tf.cast(
                        tf.reduce_mean((feat[:, i, :] - feat[:, i + 1, :]) ** 2),
                        tf.float32,
                    )
    ...
    def get_goal(self, obs, training=False):
        if self._config.gc_input == "state":
            ...
            if (not training) or self._config.training_goals == "env":
                ...
                self._config.training_goals == "batch"
                and self._config.gc_input == "skills"
            ):
                sh = obs["image_goal"].shape
                if len(sh) == 4:
                    return tf.random.uniform(
                        (sh[0], self._config.skill_dim), -1, 1, dtype=self._float
                    )
                elif len(sh) == 5:
                    return tf.random.uniform(
                        (sh[0], sh[1], self._config.skill_dim), -1, 1, dtype=self._float
                    )
                ...
                if self._config.labelled_env_multiplexing:
                    ...
                    cooc = tf.gather(
                        oh_ids, env_ids, axis=1
                    )  # co-occurence matrix, mat_ij = 0 <==> i,j are the same env
                    # Sample from the same env
                    ids = tf.random.categorical(
                        tf.math.log(cooc / tf.reduce_sum(cooc, 1, keepdims=True)), 1
                    )[:, 0]
