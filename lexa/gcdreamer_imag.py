...
import numpy as np
from tools import (
    get_data_for_off_policy_training,
    get_future_goal_idxs,
    get_future_goal_idxs_neg_sampling,
)
...
class GCDreamerBehavior(models.ImagBehavior):
    ...
    def __init__(self, config, world_model, stop_grad):
        ...
        if self._config.gc_reward == "dynamical_distance":
            ...
            self.dynamical_distance = networks.GC_Distance(
                out_dim=dd_out_dim,
                input_type=self._config.dd_inp,
                normalize_input=self._config.dd_norm_inp,
            )
            self.dd_cur_idxs, self.dd_goal_idxs = get_future_goal_idxs(
                seq_len=self._config.imag_horizon,
                bs=self._config.batch_size * self._config.batch_length,
            )
            self._dd_opt = tools.Optimizer(
                "dynamical_distance_opt",
                config.value_lr,
                config.opt_eps,
    ...
    def _gc_reward(self, feat, inp_state=None, action=None, obs=None):
        ...
        if self._config.gc_input == "embed":
            ...
            if self._config.gc_reward == "l2":
                ...
                if self._config.dd_inp == "feat":
                    ...
                    goal_feat = tf.vectorized_map(
                        self._world_model.get_init_state_embed, goal_embed
                    )["stoch"]
                ...
                if self._config.dd_loss == "regression":
                    ...
                    reward = -tf.squeeze(
                        tf.math.reduce_sum(dd_out * np.arange(self.dd_out_dim), axis=-1)
                    )
                    if self._config.dd_distance == "steps_to_go":
        ...
        # skill space goal
            ...
            if self._config.skill_pred_noise > 0:
                noise = (
                    tf.random.normal(rp_inp.shape, dtype=self._world_model._float)
                    * self._config.skill_pred_noise
                )
                rp_inp = rp_inp + noise
    ...
    def get_dynamical_distance_loss(self, _data, corr_factor=None):
        ...
        def _helper(cur_idxs, goal_idxs, distance):
            ...
            pred = tf.cast(
                self.dynamical_distance(tf.concat([cur_states, goal_states], axis=-1)),
                tf.float32,
            )
            ...
            if self._config.dd_loss == "regression":
                _label = distance
                if (
                    self._config.dd_norm_reg_label
                    and self._config.dd_distance == "steps_to_go"
                ):
        ...
        if self._config.dd_neg_sampling_factor > 0:
            num_negs = int(
                self._config.dd_neg_sampling_factor * self._config.dd_num_positives
            )
    ...
    def train(self, start, imagine=None, tape=None, repeats=None, obs=None):
        ...
        with tape or tf.GradientTape(
            persistent=self._config.gc_input == "skills"
        ) as actor_tape:
            ...
            target, weights = self._compute_target(
                actor_inp, reward, actor_ent, state_ent, self._config.slow_actor_target
            )
        ...
        if self._config.slow_value_target != self._config.slow_actor_target:
            target, weights = self._compute_target(
                actor_inp, reward, actor_ent, state_ent, self._config.slow_value_target
            )
        ...
        if self._config.gc_input == "skills":
            metrics.update(
                self._rp_opt(actor_tape, rp_loss, [self._world_model.rev_pred])
            )
            del actor_tape

        if (
            self._config.gc_reward == "dynamical_distance"
            and self._config.dd_train_imag
        ):
            with tf.GradientTape() as df_tape:
                _inp = (
                    imag_state["stoch"]
                    if "feat" in self._config.dd_inp
    ...
    def _imagine(self, start, policy, horizon, repeats=None, goal=None):
        ...
        states = {k: tf.concat([start[k][None], v[:-1]], 0) for k, v in succ.items()}
        if repeats:
