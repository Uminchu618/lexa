...
class GC_Critic(tools.Module):
    ...
    def __init__(self, act=tf.nn.relu, layers=4, units=128):
...
class GC_Actor(tools.Module):
    ...
    def __init__(self, size, act=tf.nn.relu, layers=10, units=128, from_images=True):
        self._size = size
        self._layers = layers
        self._num_layers = layers
        self._units = units
...
class DenseHead(tools.Module):
    ...
    def __init__(self, shape, layers, units, act=tf.nn.elu, dist="normal", std=1.0):
...
def get_mlp_model(name, hidden_units, out_dim):
    with tf.name_scope(name):
        ...
        for units in hidden_units:
            model.add(tfkl.Dense(units, activation="elu"))
        model.add(tfkl.Dense(out_dim, activation="tanh"))
    return model
