...
class WorldModel(tools.Module):
    ...
    def __init__(self, step, config):
        ...
        encoder_cls = dict(
            vanilla=networks.ConvEncoder, with_state=networks.ConvEncoderWithState
        )[config.encoder_cls]
...
class ImagBehavior(tools.Module):
    ...
    def __init__(self, config, world_model, stop_grad_actor=True, reward=None):
        ...
        self.value = networks.DenseHead(
            [], config.value_layers, config.units, config.act, config.value_head
        )
    ...
    def train(
        ...
        with tape or tf.GradientTape() as actor_tape:
            ...
            target, weights = self._compute_target(
                imag_feat, reward, actor_ent, state_ent, self._config.slow_actor_target
            )
        ...
        if self._config.slow_value_target != self._config.slow_actor_target:
            target, weights = self._compute_target(
                imag_feat, reward, actor_ent, state_ent, self._config.slow_value_target
            )
    ...
    def _compute_target(self, imag_feat, reward, actor_ent, state_ent, slow):
        ...
        if "discount" in self._world_model.heads:
            discount = self._world_model.heads["discount"](imag_feat, tf.float32).mean()
        else:
            discount = self._config.discount * tf.ones_like(reward)
        if self._config.future_entropy and tf.greater(self._config.actor_entropy(), 0):
